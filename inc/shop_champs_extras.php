	...
	function inc_shop_champs_extras_dist ($defaut = array()) {
		...
		if ($accepter_conditions_texte) {
			...
			foreach (array(
				...
				if ($id = sql_getfetsel('id_' . $objet, 'spip_' . $objet . 's',
						'id_' . $objet . ' IN (' . implode(',', $id_objet) . ')')) {
		...
		if ($url_cg)
			$url = '<a href="' . $url_cg . '">' . _T('shop:conditions_generales') .
			'</a>';
			else
				...
				$champs_extras = array(
					'commande' => array(
						...
						'saisies' => array(
							array(
								'type' => 'champ_outil', // Permet de distinguer les champs normaux
								// de ceux qui servent à construire le formulaire
							...
							array(
								...
								'options' => array(
									...
									'label_2' => _T('shop:label_accepter_conditions_public',
											array(
												'url' => $url
							...
							array(
								...
								'options' => array(
									...
									'label' => _T('saisies:option_obligatoire_label'),
									'defaut' => isset(
											$defaut['accepterconditions_obligatoire']) ? $defaut['accepterconditions_obligatoire'] : '',
						...
						'saisies' => array(
							array(
								...
								'options' => array(
									...
									'label' => _T(
											'shop:cfg_titre_authentification_automatique'),
									'explication' => _T(
											'shop:explication_authentification_automatique'),
									'defaut' => isset(
											$defaut['authentification_automatique']) ? $defaut['authentification_automatique'] : ''
								)
							)
						)
