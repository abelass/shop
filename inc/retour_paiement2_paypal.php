	...
	if (!$fp) {
		...
		while (!feof($fp)) {
			...
			if (strcmp ($res, "VERIFIED") == 0) {
				...
				$token_client = sql_fetsel("token","spip_shop_commande","token = '".$token."'");
				...
				* */
				...
				// enregistrement dans la bd, les log et définition des variables mail selon les réponses de paypal
					...
					case 'Canceled-Reversal' :
						...
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Completed' :
						...
						$verifier=sql_fetsel('code_devise,prix,frais_livraison','spip_shop_commande',Array("token = '".$contexte["token"]."'")); 
						...
						if(count($erreur)==0){					
							...
							sql_updateq("spip_shop_commande",Array("statut_paiement"=>"paye"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Expired' :
						...
						ql_updateq("spip_shop_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
					...
					case 'In-Progress' :
						...
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"pendant"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Part-Ref' :
						...
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"rembourse_partiellement"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Pending' :
						...
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"attente"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Refunded' :
						...
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"rembourse"),Array("token = '".$contexte["token"]."'"));
					...
					case 'Voided' :
						spip_log("NOTIFICATION Paiement d'un panier accepte","shop");
						sql_updateq("spip_shop_commande",Array("statut_paiement"=>"1_Voided"),Array("token = '".$contexte["token"]."'"));
						/*notifier_client($token_client['token_client'], _T("paypal:votre_paiement_a_bien_ete_valide_depuis_le_site_de_paypal"));
						notifier_marchand(_T("paypal:un_paiement_a_bien_ete_valide_depuis_le_site_de_paypal")."\n Voir le panier: ".generer_url_ecrire('echoppe_edit_panier','token='.$contexte["token"],"&"));
